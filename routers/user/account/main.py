...
@router.post('/', description='', response_model=Union[schemas.User, list], status_code=201, name='User Account')
...
@router.patch('/{id}', description='', name='User Account')
async def update(id:int, payload:schemas.UpdateUser, db:Session=Depends(get_db)):
    ...
    return await crud.user.update(id, payload, db)
