...
@router.post('/', response_model=schemas.Proposal, status_code=201, name='Proposal')
...
@router.get('/', response_model=schemas.ProposalList, name='Proposal')
@ContentQueryChecker(crud.proposal.model.c(), None)
async def read(db:Session=Depends(get_db), **params):
    return await crud.proposal.read(params, db)

@router.get('/{id}', response_model=Union[schemas.Proposal, dict], name='Proposal')
...
@router.patch('/{id}', response_model=schemas.Proposal, name='Proposal')
...
@router.delete('/{id}', name='Proposal')
