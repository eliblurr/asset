...
@router.post('/packages', response_model=schemas.Package, status_code=201, name='Package') # is authenticated, perm role in [admin]
...
@router.get('/packages', response_model=schemas.PackageList, name='Package') # is authenticated
@ContentQueryChecker(crud.package.model.c(), None)
async def read(db:Session=Depends(get_db), **params):
    return await crud.package.read(params, db)

@router.get('/packages/{id}', response_model=Union[schemas.Package, dict], name='Package') # is authenticated
...
@router.patch('/packages/{id}', response_model=schemas.Package, name='Package') # is authenticated, perm role in [admin]
...
@router.delete('/packages/{id}', name='Package', status_code=204) # is authenticated, perm role in [admin]
async def delete(id:int, db:Session=Depends(get_db)):
    await crud.package.delete(id, db)

@router.post('/', response_model=schemas.Subscription, status_code=201, name='Subscription') # is authenticated, perm role in [admin]
...
@router.get('/', response_model=schemas.SubscriptionList, name='Subscription') # is authenticated
@ContentQueryChecker(crud.subscription.model.c(), None)
async def read(db:Session=Depends(get_db), **params):
    return await crud.subscription.read(params, db)

@router.get('/{asset_id}/{package_id}', response_model=Union[schemas.Subscription, dict], name='Subscription') # is authenticated
...
@router.patch('/{asset_id}/{package_id}', response_model=schemas.Subscription, name='Subscription') # is authenticated, perm role in [admin]
...
@router.delete('/{asset_id}/{package_id}', name='Subscription', status_code=204) # is authenticated, perm role in [admin]
async def delete(asset_id:int, package_id:int, db:Session=Depends(get_db)):
    await crud.subscription.bk_delete_2(db, asset_id=asset_id, package_id=package_id)
