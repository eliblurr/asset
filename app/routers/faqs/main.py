...
async def create(payload:schemas.CreateFAQ, db:Session=Depends(get_db), is_validated=Depends(validate_bearer)): # , is_validated=Depends(validate_bearer)
...
async def update(id:int, payload:schemas.UpdateFAQ, db:Session=Depends(get_db), is_validated=Depends(validate_bearer)):# , is_validated=Depends(validate_bearer)
    return await crud.faq.update(id, payload, db)

@router.delete('/{id}', name='FAQ', status_code=204) # is authenticated, is system admin
async def delete(id:int, db:Session=Depends(get_db), is_validated=Depends(validate_bearer)): # , is_validated=Depends(validate_bearer)
